...
const form = el('form#payment-form.flex.flex-col.items-center.w-1/4', [
    el('div.mb-4.w-full', [
        el('label.text-gray-600', 'Номер карты'), [cardNumberInputContainer],
        cardNumberError,
    ]),
    el('div.mb-4.flex.w-full', [
        el('div.w-1/2.pr-2', [
            el('label.text-gray-600', 'Дата окончания'), [expiryInput],
            expiryError,
        ]),
        el('div.w-1/2.pl-2', [
            el('label.text-gray-600', 'CVC/CVV'), [cvcInput],
            cvcError,
        ]),
    ]),
    el('div.mb-4.w-full', [
        el('label.text-gray-600', 'Email'), [emailInput],
        emailError,
    ]),
    payButton,
]);
...
setChildren(document.body, [element, pageTitle, form]);
