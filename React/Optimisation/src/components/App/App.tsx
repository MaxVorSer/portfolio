...
function App() {
  ...
  const calculateProfitSum = (month: number) =>  month * profit.reduce((acc, i) => acc + i, 0);

  const calculateProfitSumMemo = useMemo (()=>calculateProfitSum(month), [month, profit])
  ...
  const onDecYear = () =>  setYear((count) => (count === 2018 ? 2018 : count - 1));

const MemoBestEmployees = memo(BestEmployees);
const MemoTeam = memo(Team);
  return (
    <div>
      ...
      <div className="block">
        <p>Date is: {date}</p>
        <p>Profit sum is: {calculateProfitSumMemo} $</p>
      </div>
